---
description: Capture durable handoff context into scratch/agent ledgers
argument-hint: "[focus]"
---
...
This is a context-management command. Optimize for the next agent's fastest safe resume, not for audit history.
...
Treat `scratch/agent/` as curated working memory. You may and should prune aggressively when it improves resume clarity.
...
- Validation only when it changes how to resume, e.g. known failing command, missing test, or specific regression risk.
...
- IDs/log lines/request IDs unless necessary.
...
Do not include a long duplicate of the ledger contents in the chat.
